...
class L96M:
  ...
  def f0_old(_s, z):
      return _s.rhs(z.T, t=0).T


  def f0(_s, z):
      foo_out = torch.zeros_like(z)
  ...
  def simulate(_s, slow):
    if _s.debug:
        bp()
    if _s.share_gp:
        foo = torch.empty(slow.shape)
        for k in range(_s.K):
            # foo[k] = _s.predictor(slow[None,k])
            foo[k] = _s.predictor(slow[k].reshape(1,-1))
        # return np.reshape(_s.predictor(_s.apply_stencil(slow)), (-1,))
    else:
        foo = np.reshape(_s.predictor(slow.reshape(1,-1)), (-1,))
  ...
  def compute_Yk(_s, z):
    ...
    if z.ndim > 1 and z.shape[1] > 1:
        out = z[_s.K:].reshape( (_s.J, _s.K, z.shape[1]), order = 'F').sum(axis = 0) / _s.J
    else:
        out = z[_s.K:].reshape( (_s.J, _s.K), order = 'F').sum(axis = 0) / _s.J
