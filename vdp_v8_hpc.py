...
from dynamical_models import VDP_torch
...
if FLAGS.hpc:
    base_dir = '/groups/astuart/mlevine/contRNN/vdp'
else:
    base_dir = 'output_vdp'
...
# dt=0.01
if FLAGS.multi_traj:
    train_path = os.path.join(local_path,'data/X_train_VDP_multi_traj_short.npy')
else:
    train_path = os.path.join(local_path,'data/X_train_VDP_longer.npy')
test_path = os.path.join(local_path,'data/X_test_VDP.npy')
...
train_model(my_rnn, X_train.T, X_val.T,
            ...
            logger=logger,
            dt_data=FLAGS.dt,
            lambda_endpoints=FLAGS.lambda_endpoints,
